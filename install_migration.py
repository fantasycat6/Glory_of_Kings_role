...
"""
数据库迁移API安装脚本
用于将数据库迁移API添加到admin.py文件末尾
"""

import os


def install_migration_api():
    """安装数据库迁移API"""
...
'''
    ...
    # 读取admin.py文件
    admin_py_path = os.path.join(os.path.dirname(__file__), 'admin.py')
    
    if not os.path.exists(admin_py_path):
        print("❌ admin.py 文件不存在！")
        return False
    ...
    if not content.rstrip().endswith(':'):
        print("❌ admin.py 文件末尾格式不正确")
        return False
    
    # 添加迁移API代码
    new_content = content + migration_code + '\n'
